...
docker rm -f ioc_ramp ioc_noise
...
docker run -itd --net=host -v $PWD/../db:/db --name ioc_ramp  ornl_epics/epics_prod softIocPVA -d /db/ramp.db
docker run -itd --net=host -v $PWD/../db:/db --name ioc_noise ornl_epics/epics_prod softIocPVA -d /db/noise.db
